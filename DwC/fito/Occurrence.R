...
MatrizConteo<-readxl::read_excel("datos_ERFEN_2023_long_tercer_procesado.xlsx", sheet = "Conteo")
Matriz_Densidad<-readxl::read_excel("datos_ERFEN_2023_long_tercer_procesado.xlsx", sheet = "Densidad")
...
union<-base::cbind(MatrizConteo_long, Matriz_Densidad_long[,3])
...
#occurrenceID####
occurrenceID_secuencia <- as.data.frame(sprintf("%05d", seq(1, 28197)))
...
#basisOfRecord####
basisOfRecord<-as.data.frame(rep("HumanObservation", time=28197))
colnames(basisOfRecord)="basisOfRecord"

#type####

type<-as.data.frame(rep("Event", time=28197))
colnames(type)="type"
...
accessRights<-as.data.frame(rep("Sólo para uso no comercial", time=28197))
...
ownerInstitutionCode<-as.data.frame(rep("Centro de Investigaciones Oceanográficas e Hidrográficas del Pacífico - Dirección General Marítima (DIMAR)", time=28197))
colnames(ownerInstitutionCode)="ownerInstitutionCode"


#recordNumber####
recordNumber<-as.data.frame(rep("Solicitud 31", time=28197))
colnames(recordNumber)="recordNumber"

#recordedBy####
recordedBy<-as.data.frame(rep("Fredy Albeiro Castrillón Valencia", time=28197))
colnames(recordedBy)="recordedBy"

#recordedByID####

recordedByID<-as.data.frame(rep("https://orcid.org/0000-0003-2815-6122", time=28197))
...
organismQuantityType<-as.data.frame(rep("Células por litro", time=28197))
...
#identifiedBy####
identifiedBy<-as.data.frame(rep("Humberto Luis Quintana-Manotas", time=28197))
colnames(identifiedBy)="identifiedBy"

#identifiedByID####

identifiedByID<-as.data.frame(rep("https://orcid.org/0000-0002-1700-4476", time=28197))
colnames(identifiedByID)="identifiedByID"


#dateIdentified####

dateIdentified<-as.data.frame(rep("2024-04-01/2024-06-30", time=28197))
...
#Taxonomía####
taxonomia<-readxl::read_excel("datos_ERFEN_2023_long_tercer_procesado.xlsx", sheet = "listadoTaxonomico")
